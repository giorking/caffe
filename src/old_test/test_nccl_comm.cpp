...
void ThreadEntrySingleNode(SyncCommConfig<Dtype>& config, int n_device) {
...
void TestNCCLComm() {
  ...
  vector<SyncCommConfig<float> > configs;
  ...
  for (int i = 0; i < gpu_ids.size(); i++) {
    configs.push_back(SyncCommConfig<float> (i, 0, gpu_ids.size(), 
      i, 0, clique_id, false, comm_buffer_size, comm_buffer_size) ); 
  }
